...
export const Button = styled.button<secondaryButtonProps>`
  min-width: 80px;
  height: 32px;
  background: ${(props) => props.buttonStyle === 'outlined' ? palletCollors.white :  palletCollors.secondary};
  color: ${(props) => props.buttonStyle === 'outlined' ?  palletCollors.secondary : palletCollors.white };
  font-weight: bold;
  border-radius: 4px;
